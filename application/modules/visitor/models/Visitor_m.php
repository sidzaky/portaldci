	...
	public function __construct()
   {
	...
	public function checkin_m(){
		if (sizeof($this->visitor_m->checkin_cek_m($_POST['id_visitor']))==0){
			...
			$_POST['time_in']=time();
			$_POST['user_input']=$this->session->userdata('user_id');
			$this->db->insert('visitor_log_gedung_temp', $_POST);
	...
	public function checkout_m(){
		$getdata="select * from visitor_log_gedung_temp where id_log_gedung='".$_POST['id_log_gedung']."'";
		foreach ($this->db->query($getdata)->result_array() as $row){
			//update table permanent log
			$row['time_out']=time();
			$row['user_input_out']=$this->session->userdata('user_id');
			$this->db->insert('visitor_log_gedung',$row);
			...
			//update table visitor key
			$sql="update visitor_key set available=1 where id_key='".$_POST['id_key']."'";
			$this->db->query($sql);
			
			//update table visitor list (menambahkan temp log biar lebih cepat)
			$sql="update visitor_list set last_log_checkin='".date("H:i d M, Y", $row['time_in'])." - ".date("H:i d M, Y", $row['time_out'])." (".preg_replace('/[^A-Za-z0-9\-]/', ' ',$row['bussiness']).")' where id_visitor='".$row['id_visitor']."'";
			$this->db->query($sql);
			
			//menghapus data table log temporary
			$sql="delete from visitor_log_gedung_temp where id_log_gedung='".$_POST['id_log_gedung']."'";
			$this->db->query($sql);
	...
	public function checkin_cek_m($id){
		$sql="select a.*, b.warna, b.keterangan, b.nomer from visitor_log_gedung_temp a
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/////////////////////////////// visitor management model start /////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	
	public function get_total_inside(){
		$sql="select count(a.id_visitor) from visitor_log_gedung_temp a 
    ...
    var $column_search = array('concat(c.warna,c.nomer)','name_visitor','id_number','company','phone_number', " IF ( ( UNIX_TIMESTAMP( NOW( ) ) - b.time_in ) > 86400, ' selama ', ' idkey ' ) " ); //field yang diizin untuk pencarian 
    var $order = array('timeinput' => 'asc'); // default order 
 
    function get_datatables()
    {
        $i=0;
		$sql="	SELECT DISTINCT	(a.id_visitor),
								c.*,
								a.*,
								b.id_log_gedung,
								b.bussiness,
								b.time_in,
								d.username AS user_in
				from visitor_list a
				left join visitor_log_gedung_temp b on a.id_visitor=b.id_visitor
				left join visitor_key c on b.id_key=c.id_key
				left join user d on b.user_input=d.id
				where a.active='1' ";
				
		if ($this->session->userdata('module_access')['vm1']<3){
	...
	public function visitor_upload_nda_m($data){
		$this->db->where('id_visitor',$_POST['id_visitor']);
		$this->db->update('visitor_list',$data);
		$message= json_encode($data);
		$this->activity_m->writelog('visitor','update nda {'.$message.'}');
	}
	
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/////////////////////////////// visitor management model end///////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	
	
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	////////////////////////////////////// DC Request model start ///////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	
	
	var $dcrequest_m_column_search = array('b.name_visitor');
	
	function dcrequest_m_get($approvall=""){
        $i=0;
		$sql="	SELECT a.*,b.*, c.USERNAME, e.nama_dc, f.keterangan_area, c.BAGIAN, d.USERNAME as user_approval from visitor_schedule_dc a
					inner join visitor_list b on a.id_visitor=b.id_visitor
					left join user c on a.user_input=c.ID
					...
					left join asset_dc e on e.id_asset_dc=a.dc
					left join asset_dc_area f on f.id_asset_dc_area=a.access
					Where b.active !=0 ".$approvall;
				
		if ($this->session->userdata('module_access')['vm2']==4) $sql.=" and a.user_input='".$this->session->userdata('user_id')."' ";
		 	
		foreach ($this->dcrequest_m_column_search as $item)
			{
				if($_POST['search']['value']!="") 
					{	
						...
						if(count($this->dcrequest_m_column_search) - 1 == $i) 
							$sql .= " ) ";
					}
				$i++;
			}
	...
	public function dcrequest_m(){
			$sql  = $this->dcrequest_m_get(" and approval='' and range_end>=UNIX_TIMESTAMP(CURDATE()) ");
			if($_POST['search']['value']!=""){
				$sql .="  ";
			}
			else {
	...
	public function dcrequest_m_count_all(){	
		$sql  = $this->dcrequest_m_get();
        return  $this->db->query($sql)->num_rows();
    }
	
	
	public function dcresponse_server_side_m(){
			$sql  = $this->dcrequest_m_get(" and approval!='' ");
			if($_POST['search']['value']!=""){
				$sql .=" limit 20 ";
			}
			else {
	...
	var $dcform_m_column_search = array('name_visitor','id_number','phone_number');
	  
	 function dcform_m_get(){
		    $i=0;
			$sql="	SELECT * from visitor_list where active='1' ";
			foreach ($this->dcform_m_column_search as $item) {
				if($_POST['search']['value']!="") {
						if($i===0) {	
							$sql .= 'and ('.$item.' LIKE "%'.$_POST['search']['value'].'%" ESCAPE "!" ';
						}
						...
						if(count($this->dcform_m_column_search) - 1 == $i) $sql .= " ) ";
					}
				$i++;
			}
			return $sql;
	...
	public function dcform_m(){
		$sql= $this->dcform_m_get();
		if($_POST['search']['value']!="") $sql .=" limit 5 ";
		else $sql .=" limit 0 ";
		return $this->db->query($sql);
	}
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	////////////////////////////////////// DC Request model end  ///////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	
	
	
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	////////////////////////////////////// DC CheckIn checkout model start ////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	
	
	var $dcgateway_server_side_column_search = array('name_visitor'); //field yang diizin untuk pencarian 
	function dcgateway_server_side_get(){
        $i=0;
		$sql="SELECT 	a.*,
								b.*,
								f.id_log_dc,
								...
								c.USERNAME
							FROM
								visitor_schedule_dc a
								INNER 	JOIN visitor_list b ON a.id_visitor = b.id_visitor
								LEFT 	JOIN visitor_log_dc_temp f on a.id_schedule_dc=f.id_schedule_dc
								LEFT 	JOIN user c ON a.user_input = c.ID
								LEFT 	JOIN asset_dc d ON d.id_asset_dc = a.dc
								LEFT 	JOIN asset_dc_area e ON e.id_asset_dc_area = a.access 
											  Where b.active !=0 and approval='1'  ";
			foreach ($this->dcgateway_server_side_column_search as $item) // looping awal
			{
				if($_POST['search']['value']!="") // jika datatable mengirimkan pencarian dengan metode POST
					{	
						if($i===0) // looping awal
						...
						if(count($this->dcgateway_server_side_column_search) - 1 == $i) 
							$sql .= " ) ";
					}
				$i++;
			}
			///////////////////////////////////////// filter by site data center
			/////////////////////////////////////////
			$sql .= " and  range_start<=UNIX_TIMESTAMP(CURDATE()) and 
								range_end>=UNIX_TIMESTAMP(CURDATE())
						order by  f.time_in desc, a.time_input desc ";
        return $sql;
    }
	
	
	
	public function dcgateway_server_side_m(){
		$sql  = $this->dcgateway_server_side_get();
		if($_POST['search']['value']!="") {
			$sql .=" limit 200 ";
		}
		else {
	 ...
	 public function dcgateway_server_side_count_all(){	
		$sql  = $this->dcgateway_server_side_get();
        return  $this->db->query($sql)->num_rows();
    }
	...
	public function dc_checkin_m(){
		...
		$_POST['user_input']=$this->session->userdata('user_id');
		$_POST['nda']='1';
		$this->db->insert('visitor_log_dc_temp', $_POST);
		$this->activity_m->writelog('visitor','visitor checkin on Data Center id_log_dc='.$_POST['id_log_dc'].'');
	}
	
		
	public function dc_checkout_m(){
		$first="select * from visitor_log_dc_temp where id_log_dc='".$_POST['id_log_dc']."'";
		foreach ($this->db->query($first)->result_array() as $row){
				$row['user_input_out']=$this->session->userdata('user_id');
				$row['time_out']=time();
				$this->db->insert('visitor_log_dc', $row);
				$this->activity_m->writelog('visitor','visitor checkout from Data Center id_log_dc='.$_POST['id_log_dc']);
		}
		$del="delete from visitor_log_dc_temp where id_log_dc='".$_POST['id_log_dc']."'";
		$this->db->query($del);
	}
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	////////////////////////////////////// DC CheckIn checkout model end ////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	
	
	
	public function upload_document_m($data){
		$sql="update visitor_list set document_".$_POST['type']."='".$data['document']."', document_".$_POST['type']."_timeinput='".time()."', document_".$_POST['type']."_userinput='".$this->session->userdata('user_id')."'  where id_visitor='".$data['id_visitor']."'";
		$this->db->query($sql);
		$message='upload new document_'.$_POST['type'].' with id_visitor='.$data['id_visitor'].' by user id '.$this->session->userdata('user_id') ;
