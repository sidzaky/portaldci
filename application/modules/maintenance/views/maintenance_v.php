...
<script>
		...
		function asset_select(i){
			...
			$.ajax({ type:"POST",
				   url: "<?php echo base_url();?>maintenance/maintenance_addasset",
				   data: data1,
		...
		function disasset(i,k){
					if (confirm('yakin menghapus '+k+' dalam daftar list maintenance????')){
						...
						$.ajax({ type:"POST",
									   url: "<?php echo base_url();?>maintenance/maintenance_delasset",
									   data: data1,
									   success:function(msg){
											$('.x_content').html(msg);
											}
		...
		function input_maintenance(i){
						if (i==null){
							var address="<?php echo base_url();?>maintenance/input_maintenance";
							var data1=null;
						}
						else{ 
							var address="<?php echo base_url();?>maintenance/get_update_maintenance";
							var data1= {
										'ID' :  i,
									};
							}
		...
		function delfile(i,k,l){
					if (confirm('yakin menghapus file '+k+'???')){
						...
						$.ajax({ type:"POST",
									   url: "<?php echo base_url();?>maintenance/delfile",
									   data: data1,
									   success:function(msg){
											$('.x_content').html(msg);
											}
		...
		function delmaintenance(i,k){
					if (confirm('yakin menghapus '+k+'???')){
						...
						$.ajax({ type:"POST",
									   url: "<?php echo base_url();?>maintenance/delmaintenance",
									   data: data1,
									   success:function(msg){
											$('.x_content').html(msg);
											}
		...
		function uploadfile(i){
			...
			if (files && file) {
				...
				reader.onload = function(readerEvt) {
					...
					$.ajax({ type:"POST",
						   url: "<?php echo base_url();?>maintenance/upload_document_maintenance",
						   data: data1,
						   success:function(msg){
								$('.x_content').html(msg);
								}
