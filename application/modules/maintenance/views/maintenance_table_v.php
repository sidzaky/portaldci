					  ...
					  $i=0;
						foreach ($maintenance as $row){
							...
							foreach ($con->maintenance_m->getmaintenanceasset_file($row['ID']) as $srow){
									$file.='<span class="label label-lg label-info">&nbsp;
										<a class="white" target="_blank" href="'.base_url().$srow['path_file'].'">
											...
											<span>'.substr($srow['name_view'],0, 10).'</span>
										</a>
							...
							foreach ($con->maintenance_m->getmaintenanceasset_list($row['ID']) as $drow){
								$assetlist.='<span class="label label-lg label-primary">&nbsp;<span>'.$drow['HOSTNAME'].'&nbsp;&nbsp; &nbsp;&nbsp;</span><a href="#" onclick="disasset(\''.$drow['id_asset_maintenance_list'].'\',\''.$drow['HOSTNAME'].'\');" title="Removing tag"><i class="fa fa-close red"></i></a></span>';	
								if ($z==2) { $assetlist.='</br>'; $z=0;}
								else $z++;
							}
							$file.='';
							echo '<tr>
									...
									<td>'.date('d-M, Y' , $row['EVENT_DATE']).'</td>
									<td>'.$assetlist.'</br><button class="btn btn-success btn-sm" data-toggle="modal" data-target=".modal-list-asset" type="button"  onclick="activemaintenance=\''.$row['ID'].'\';"><i class="fa fa-plus"></i></button> </br></br></td>
									<td>'.$file.'</br>
										'.($this->session->userdata('module_access')['maintenance']>=2 ? '<button class="btn btn-success btn-sm" type="button" '.$disabled.' onclick="buttonup(\''.$row['ID'].'\');"><span class="btn-label"><i class="fa fa-upload"></i></span>Dokument/Foto </button>' : '').'
